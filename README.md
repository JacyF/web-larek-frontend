...
#### **Атрибуты**
- `_productCards: IProduct[]` - Массив объектов карточек продуктов.
- `selectedСard: IProduct` - Выбранная карточка продукта, которая отображается в модальном окне.
...
#### **Методы:**  
- `setOrderData` - Обновляет данные формы заказа для указанных полей.
- `getOrderLot` - Возвращает объект, содержащий текущие данные формы заказа.
...
- `basketPrice (HTMLElement)` — Элемент отображения общей стоимости товаров.
...
#### **Атрибуты**
- `formOrder: HTMLFormElement` - HTML-элемент формы оформления заказа.
- `buttonAll: HTMLButtonElement[]` - Массив кнопок для выбора способа оплаты.
...
#### **Атрибуты**
- `formContacts: HTMLFormElement` - HTML-форма для ввода контактной информации.
- `inputAll: HTMLInputElement[]` - Массив всех полей ввода в форме (например, email, телефон).
- `buttonSubmit: HTMLButtonElement` - Кнопка для подтверждения и отправки формы.
- `events: IEvents` - Объект для управления пользовательскими событиями.
...
- `_content: HTMLElement` - Элемент, представляющий содержимое модального окна.
- `events: IEvents` - Объект для управления пользовательскими событиями.
...
#### **Атрибуты**
- `success: HTMLElement` - Корневой элемент экрана успешного оформления заказа.
- `description: HTMLElement` - Элемент, отображающий описание с информацией о списанных средствах.
- `button: HTMLButtonElement` - Кнопка для закрытия экрана успешного оформления заказа.
- `events: IEvents` - Объект для управления пользовательскими событиями.
